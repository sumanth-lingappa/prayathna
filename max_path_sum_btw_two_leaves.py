"""
Source: https://www.geeksforgeeks.org/find-maximum-path-sum-two-leaves-binary-tree/
...
# 1) Maximum path sum between two leaves which are stored in result
# 2) The maximum root to leaf path sum which is returned
# If one side of root is empty, then it returns INT_MIN
def maxPathSumUtil(root, result):
    # Base cases
    ...
    ls = maxPathSumUtil(root.left, result)
    rs = maxPathSumUtil(root.right, result)
    ...
    if root.left is None: return rs + root.data
    elif root.right is None: return ls + root.data
